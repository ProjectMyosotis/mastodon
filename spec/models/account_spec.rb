  ...
  describe 'validations' do
    ...
    context 'when is local' do
      ...
      it 'is invalid if the username is longer then 128 characters' do
        account = Fabricate.build(:account, username: Faker::Lorem.characters(number: 129))
        account.valid?
        expect(account).to model_have_error_on_field(:username)
      end

      it 'is invalid if the display name is longer than 128 characters' do
        account = Fabricate.build(:account, display_name: Faker::Lorem.characters(number: 129))
        account.valid?
    ...
    context 'when is remote' do
      ...
      it 'is valid even if the username is longer then 128 characters' do
        account = Fabricate.build(:account, domain: 'domain', username: Faker::Lorem.characters(number: 129))
        account.valid?
